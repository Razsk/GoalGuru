...
* **Context Exported**: Target Action, its ancestor spine, attached evidence, and immediate dependency outputs.
...
* **Expected Output**: Change Set containing `update_status` mutations (marking finished actions `done`), `add_evidence` for recorded notes/outcomes, and `create_node` for any newly discovered follow-up actions.
...
* **Intent**: Audit the dependency graph of a Goal to detect missing dependencies, implicit assumptions, circular dependencies, or bottleneck actions on the critical path.
...
### 9. `next_action`
* **Intent**: Identify the optimal next 1–3 actions for the user to execute right now based on dependency readiness, priority, and momentum.
* **Context Exported**: All `ready` actions across active goals.
* **Expected Output**: Prioritized guidance in `advice` explaining why these actions should be tackled first. Does not mutate state.
...
* **Expected Output**: Comprehensive assessment in `advice` highlighting accomplishments, drift from original goals, and guidance for the next cycle.
...
* **Expected Output**: Verification assessment checking all required milestones and outputs against actual outcomes, advising whether to mark complete or addressing remaining gaps.
